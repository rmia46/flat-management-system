...
export const approveBooking = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // NEW: Check if the booking status is still pending before approving.
    if (booking.status !== 'pending') {
      return res.status(400).json({ message: 'Booking is not in pending status and cannot be approved.' });
    }
    
    // Begin a Prisma transaction to ensure both updates happen successfully
    const [updatedFlat, updatedBooking] = await prisma.$transaction([
      prisma.flat.update({
    ...
    res.status(200).json({ message: 'Booking and flat status approved successfully.', booking: updatedBooking });
  } catch (error) {
    console.error('Error approving booking:', error);
    res.status(500).json({ message: 'Server error approving booking. Please check server logs.' });
  }
};
...
// NEW: Confirm Payment Controller (Tenant only)
export const confirmPayment = async (req: Request, res: Response) => {
    ...
    try {
      const booking = await prisma.booking.findUnique({
        where: { id: parseInt(id) },
        select: { id: true, userId: true, flat: { select: { id: true, status: true } }, status: true },
      });
      ...
      // Update the flat status to 'booked'
      await prisma.flat.update({
        where: { id: booking.flat.id },
        data: { status: 'booked' },
      });

      res.status(200).json({ message: 'Payment confirmed. Booking is now active.', booking: updatedBooking });
    } catch (error) {
